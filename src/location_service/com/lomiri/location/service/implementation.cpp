...
#include <com/lomiri/location/service/session/implementation.h>
...
clls::Implementation::Implementation(const clls::Implementation::Configuration& config)
    : Skeleton
      {
          Skeleton::Configuration
          {
              config.incoming,
              config.outgoing,
              clls::Skeleton::CredentialsResolver::Ptr
              {
              ...
              {
                  new clls::Skeleton::ObjectPathGenerator{}
              },
      ...
      connections
      {
          ...
          does_report_cell_and_wifi_ids().changed().connect(
              [this](bool value)
              {
                  configuration.engine->configuration.wifi_and_cell_id_reporting_state
                      = value ?
                          cll::WifiAndCellIdReportingState::on :
                          cll::WifiAndCellIdReportingState::off;
          ...
          does_satellite_based_positioning().changed().connect(
              [this](bool value)
              {
                  configuration.engine->configuration.satellite_based_positioning_state
                          = value ?
                              cll::SatelliteBasedPositioningState::on :
                              cll::SatelliteBasedPositioningState::off;
              }),
          ...
          configuration.engine->configuration.satellite_based_positioning_state.changed().connect(
              [this](cll::SatelliteBasedPositioningState state)
              {
...
clls::session::Interface::Ptr clls::Implementation::create_session_for_criteria(const cll::Criteria& criteria)
{
    ...
    return session::Interface::Ptr{new clls::session::Implementation(proxy_provider)};
}
